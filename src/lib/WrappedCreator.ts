...
export default class WrappedCreator {
  ...
  private async getDirectMessages(
    ...
    for (const file of directMessageFiles) {
      ...
      // if (messages.participants.length == 2) {
  ...
  private async getActivity(zip: JSZip): Promise<InstagramActivity> {
    ...
    if (commentsFile) {
      ...
      // console.log(
      //   comments.map((c: any) => {
      //     if (c.string_map_data && c.string_map_data.Comment) {
      //       return {
      //         href: "",
    ...
    if (likedPostsFile) {
      ...
      output.likedPosts = this.removeOutdatedEntries(
        likedPosts.likes_media_likes.map((c: any) => ({
          href: c.string_list_data[0].href,
          value: c.string_list_data[0].value,
          timestamp: c.string_list_data[0].timestamp,
          account: c.title,
          isReel: c.string_list_data[0].href.startsWith("https://www.instagram.com/reel/") ??
          false,
          isPost: c.string_list_data[0].href.startsWith("https://www.instagram.com/p/") ??
          false,
        })) as InstagramPost[]
      );
    }
